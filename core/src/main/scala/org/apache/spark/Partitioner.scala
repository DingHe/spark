...
abstract class Partitioner extends Serializable {
  def numPartitions: Int  //返回分区的数量，通常是一个整数
  def getPartition(key: Any): Int  //根据传入的键，返回该键应该被分配到的分区ID
}
...
object Partitioner {
   ...
   * 用于选择在多个RDD进行类似cogroup操作时使用的分区器（Partitioner）
   * We use two method parameters (rdd, others) to enforce callers passing at least 1 RDD.
   */
  def defaultPartitioner(rdd: RDD[_], others: RDD[_]*): Partitioner = {
    val rdds = (Seq(rdd) ++ others)
    val hasPartitioner = rdds.filter(_.partitioner.exists(_.numPartitions > 0)) //筛选出那些已经设置了有效分区器

    val hasMaxPartitioner: Option[RDD[_]] = if (hasPartitioner.nonEmpty) { //找到拥有最大分区数的 RDD
      Some(hasPartitioner.maxBy(_.partitions.length))
    } else {
      None
    }

    val defaultNumPartitions = if (rdd.context.conf.contains("spark.default.parallelism")) { //获取 Spark 配置中 spark.default.parallelism 的值。如果该配置存在，就使用该值作为默认的分区数
      rdd.context.defaultParallelism
    } else {
      rdds.map(_.partitions.length).max //如果没有配置 spark.default.parallelism，则取所有 RDD 中分区数最多的那个 RDD 的分区数作为默认的分区数
    }

    // If the existing max partitioner is an eligible one, or its partitions number is larger
    // than or equal to the default number of partitions, use the existing partitioner.
    if (hasMaxPartitioner.nonEmpty && (isEligiblePartitioner(hasMaxPartitioner.get, rdds) ||
        defaultNumPartitions <= hasMaxPartitioner.get.getNumPartitions)) {  //当前默认的分区数小于等于最大分区器的分区数
      hasMaxPartitioner.get.partitioner.get
    } else {
      new HashPartitioner(defaultNumPartitions)  //如果条件不成立，说明需要使用一个新的分区器，这里会使用 HashPartitioner，并且默认使用之前计算出的 defaultNumPartitions 分区数
    }
  }

  /** 该方法判断某个分区器是否符合条件，通常是判断它的分区数量是否在合理范围内，即是否比最大分区数少一个数量级
...
/** 直接将键（假设是分区ID）返回，通常用于在预先计算了分区ID的场景下使用
 ...
 * 将数据根据键的范围进行分区。它会通过对数据进行采样，并根据这些采样点来划分分区的范围。它常用于对可排序的键（如整数或字符串）进行分区
